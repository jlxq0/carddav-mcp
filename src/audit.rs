...
//! event at `target: "carddav_mcp::audit"`. The cluster's Alloy collector ships
...
/// Audit-safe resource-id check: no whitespace/control chars, bounded length.
/// `CardDAV` ids are collection/object hrefs and vCard UIDs — path-shaped
/// opaque strings — anything else is caller junk.
...
mod tests {
    ...
    #[test]
    fn is_safe_id_accepts_dav_ids_and_emails() {
        assert!(is_safe_id("Mca12345"));
        assert!(is_safe_id("/dav/card/julian/default/"));
