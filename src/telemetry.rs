...
//! - `tool`, `user`, `resource` (DAV href), `outcome`, `latency_ms`
//! - `token_hash` (SHA-256 prefix, same as the Loki audit log)
//!
//! Span attributes must NOT include vCard content (names, addresses,
//! phone numbers, notes), access tokens, or any other user-supplied
    ...
    S: tracing::Subscriber + for<'span> tracing_subscriber::registry::LookupSpan<'span>,
{
        ...
        .build()
    {
        ...
        Err(err) => {
            ...
            eprintln!("carddav-mcp: OTLP exporter init failed: {err}; disabling OTel tracing");
            return None;
        }
    };
    ...
    let tracer = provider.tracer("carddav-mcp");
