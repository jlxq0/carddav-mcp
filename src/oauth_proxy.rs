...
struct Inner {
    ...
    /// Forced RFC 8707 `resource` sent to Logto. Must be an absolute URI
    /// (Logto API resource indicator). Never the bare string `stalwart`.
    dav_audience: String,
    ...
    pub fn new(
        ...
        Self {
            inner: Arc::new(Inner {
                ...
                dav_audience: dav_audience.into(),
...
/// Force the RFC 8707 `resource` we send to Logto to an absolute URI.
///
/// Clients send `{origin}` or `{origin}/mcp` (RFC 9728 advertised
/// resource). Logto requires an absolute URI (RFC 8707) and rejects
/// the bare API name `stalwart` with `invalid_target`. The live Logto
/// API resource for this proxy is `https://carddav-mcp.kampong.social`
/// (the origin). RFC 9728 metadata stays `{origin}/mcp`; only the
/// Logto `resource` parameter is rewritten.
///
/// A non-URI `dav_audience` (including `stalwart`) is refused: we
/// never emit it.
fn apply_dav_resource(pairs: &mut Vec<(String, String)>, dav_audience: &str) {
    if !crate::config::is_absolute_http_uri(dav_audience) {
        return;
    }
    let mut saw = false;
    for (k, v) in pairs.iter_mut() {
        if k == "resource" {
            dav_audience.clone_into(v);
            saw = true;
        }
    }
    if !saw {
        pairs.push(("resource".to_owned(), dav_audience.to_owned()));
    }
}
...
pub async fn authorize(State(st): State<OAuthProxyState>, RawQuery(q): RawQuery) -> Response {
    ...
    apply_dav_resource(&mut pairs, &st.inner.dav_audience);
...
mod tests {
    ...
    fn callback_url_derived_from_resource() {
        ...
        assert_eq!(
            ...
            "https://carddav-mcp.example.test/oauth/callback"
        );
    ...
    async fn authorize_forwards_only_allowed_redirect_uri_to_pending_state() {
        ...
        let response = authorize(
            State(st.clone()),
            RawQuery(Some(
                "client_id=abc&redirect_uri=https%3A%2F%2Fclaude.ai%2Fapi%2Fmcp%2Fauth_callback&state=client-state&resource=https%3A%2F%2Fcarddav-mcp.example.test%2F"
                    .to_owned(),
            )),
        )
        .await;
        ...
        assert_eq!(
            ...
            Some("https://carddav-mcp.example.test/oauth/callback")
        );
        let resource = params.get("resource").map(String::as_str);
        assert_eq!(resource, Some("https://carddav-mcp.example.test"));
        let parsed = Url::parse(resource.unwrap()).unwrap();
        assert!(parsed.scheme() == "https" && parsed.host_str().is_some());
        assert_ne!(resource, Some("stalwart"));
    ...
    #[test]
    fn apply_dav_resource_rewrites_origin_and_mcp_path_to_absolute_uri() {
        const URI: &str = "https://carddav-mcp.kampong.social";
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://carddav-mcp.kampong.social/mcp".to_owned(),
        )];
        apply_dav_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
        assert!(Url::parse(&pairs[0].1).unwrap().has_host());
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://carddav-mcp.kampong.social/".to_owned(),
        )];
        apply_dav_resource(&mut pairs, URI);
        assert_eq!(pairs, vec![("resource".to_owned(), URI.to_owned())]);
    }
    ...
    #[test]
    fn apply_dav_resource_inserts_absolute_uri_when_missing() {
        const URI: &str = "https://carddav-mcp.kampong.social";
        ...
        assert_eq!(
            ...
            vec![
                ...
                ("resource".to_owned(), URI.to_owned()),
            ]
        );
        assert_ne!(pairs[1].1.as_str(), "stalwart");
        assert!(Url::parse(&pairs[1].1).is_ok());
    }

    #[test]
    fn apply_dav_resource_never_sends_bare_stalwart() {
        let mut pairs = vec![(
            "resource".to_owned(),
            "https://carddav-mcp.kampong.social/mcp".to_owned(),
        )];
        apply_dav_resource(&mut pairs, "stalwart");
        let resource = pairs
            .iter()
            .find(|(k, _)| k == "resource")
            .map(|(_, v)| v.as_str());
        assert_ne!(resource, Some("stalwart"));
        // refused: original client value is left untouched
        assert_eq!(resource, Some("https://carddav-mcp.kampong.social/mcp"));
    }
