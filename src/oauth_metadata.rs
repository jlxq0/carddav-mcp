...
//! public-SPA client (`CARDDAV_MCP_DCR_CLIENT_ID`, redirect URIs already
...
/// `CARDDAV_MCP_RESOURCE_URL` to include `/mcp`.
...
mod tests {
    ...
    fn protected_resource_points_at_self() {
        ...
        assert_eq!(m.resource, "https://carddav-mcp.example.test/mcp");
        ...
        assert_eq!(
            ...
            vec!["https://carddav-mcp.example.test"]
        );
    ...
    fn as_metadata_is_same_origin_with_jwks_on_logto() {
        ...
        assert_eq!(m.issuer, "https://carddav-mcp.example.test");
        // authorize/token are on OUR origin (proxied); jwks stays on Logto.
        assert_eq!(
            m.authorization_endpoint,
            "https://carddav-mcp.example.test/authorize"
        );
        assert_eq!(m.token_endpoint, "https://carddav-mcp.example.test/token");
    ...
    fn as_metadata_advertises_registration_when_configured() {
        ...
        assert_eq!(
            ...
            Some("https://carddav-mcp.example.test/register")
        );
    }
    ...
    #[tokio::test]
    async fn register_accepts_cursor_and_grok_four_uri_body() {
        let mut cfg = test_config();
        cfg.dcr_client_id = Some("uw7dfhsvg6wq0p0eavk2i".to_owned());
        cfg.oauth_redirect_uris = vec![
            ...
            "grokbot://mcp/oauth/callback".to_owned(),
            "http://localhost:8787/callback".to_owned(),
            "https://www.cursor.com/agents/mcp/oauth/callback".to_owned(),
        ];
        let body = json!({
            "redirect_uris": [
    ...
    fn www_authenticate_includes_resource_metadata_url() {
        let h = www_authenticate_header("https://carddav-mcp.example.test");
        assert!(h.starts_with("Bearer "));
        assert!(h.contains(
            r#"resource_metadata="https://carddav-mcp.example.test/.well-known/oauth-protected-resource/mcp""#
        ));
    }
}
